...
def create_logger(cfg, cfg_path, phase='train',rank=-1):
    ...
    if rank in [-1, 0]: 
        ...
        tensorboard_log_dir: Path = Path(cfg.LOG_DIR) / dataset / model / (cfg_path + '_' + time_str)
        ...
        final_log_file: Path = tensorboard_log_dir / log_file
        head = '%(asctime)-15s %(message)s'
        logging.basicConfig(
            filename=str(final_log_file), format=head, level=logging.INFO)
...
def select_device(logger=None, device='', batch_size=None):
    ...
    if device and not cpu_request:
        ...
        assert torch.cuda.is_available(), f"CUDA unavailable, invalid device {device} request"
    ...
    if cuda:
        ...
        for i in range(0,ng):
            if i ==1:
                s = ' ' * len(s)
            if logger:
                logger.info(f"{s}CUDA:{i} ({x[i].name}, {x[i].total_memory / c}MB)")
    else:
        if logger:
