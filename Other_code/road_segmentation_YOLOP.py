...
class Bottleneck(nn.Module):
    def __init__(self,in_channels,out_channels,shortcut=True,groups=1,e=0.5):
        ...
        self.cv2 = Conv(mid_channels,out_channels,3,1,1)
...
class LightweightRoadSegmentationNet(nn.Module):
    def __init__(self):
        ...
        self.encoder3 = nn.Sequential(
            ...
            SimSPPF(512,512),
            BottleNeckCSP(512,512,num_Bottleneck=1),
            Conv(512,256,kernel_size=1,stride=1),
            UpSample(size=(45,80), mode='nearest')  #特别注明，如果输入是(720,1280)，这里设置size = (45,80)
            # UpSample(scale_factor=2, mode='nearest')
        )
        self.encoder4 = nn.Sequential(
            BottleNeckCSP(512, 256,num_Bottleneck=1),#拼接后输入通道数为256+256=512
        ...
        self.drivable_branch = nn.Sequential(
            ...
            UpSample(scale_factor=2, mode='nearest'),
            BottleNeckCSP(128, 64,num_Bottleneck=1),
            Conv(64, 32, kernel_size=3, stride=1),
            UpSample(scale_factor=2, mode='nearest'),
            Conv(32, 16, kernel_size=3, stride=1),
            BottleNeckCSP(16, 8,num_Bottleneck=1),
            UpSample(scale_factor=2, mode='nearest'),
            Conv(8, 1, kernel_size=3, stride=1)     #(8,2,3,1)
        )
        #车道线区域分支
    ...
    def forward(self,x):
        ...
        y2 = self.encoder2(y1)
        y3 = self.encoder3(y2)
        y4 = self.encoder4(torch.concat((y2,y3),dim=1))
        y5 = torch.concat((y1,y4),dim=1)
        #路面识别分支
